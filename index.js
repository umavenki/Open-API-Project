...
const messageEl = document.getElementById("validate");
...
function getFullForecast() {
  ...
  let isvalid = validate(latitude, longitude);
  if (isvalid == true) {
    fetch(
...
function validate(latitude, longitude) {
  ...
  if (!latitude || !longitude) {
    ...
    let flatitude = parseFloat(latitude);
    let flongitude = parseFloat(longitude);
    if ((!flatitude && flatitude != 0) || (!flongitude && flongitude != 0)) {
      valid = false;
      message = "Enter valid input Number";
    } else {
      if (
        latitude <= 90 &&
        latitude >= -90 &&
        ...
        valid = false;
        message =
          "Please enter the range from -90 to 90 for lattitude and -180 to 180 for longitude";
      }
    }
  }
  if (!valid) {
